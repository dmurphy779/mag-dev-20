...
function html5blank_header_scripts() {
    if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
        wp_register_script('html5blankscripts', get_template_directory_uri() . '/js/scripts.js', array('jquery')); // Custom scripts
	    ...
	    wp_enqueue_script( 'cookie-consent-script','//cdnjs.cloudflare.com/ajax/libs/cookieconsent2/3.1.0/cookieconsent.min.js', array(), '1.0', true );
...
function html5blank_styles() {
    ...
    wp_register_style('html5blank', get_template_directory_uri() . '/style/css/style.css', array(), 'all');
...
/* --------------------------
 * Find the Issue Category ID of a single post
 * ----------------------- */

function getIssueCategoryID($issuesID) {
    ...
    $issueCategories = get_categories(array('child_of' => $issuesID));
    $postIssueIds = wp_get_post_categories($post_id);
    $issueCategoriesIDArray = array();
    foreach ($issueCategories as $issueCategory) {
        $issueCategoriesIDArray[] = $issueCategory->cat_ID;
    }

    $result = array_values(array_intersect($postIssueIds, $issueCategoriesIDArray))[0];
    if (!empty($result)){
        return $result;
    }
    else{
        //echo 'this page is not associated with a magazine issue.';
    }
}
...
function getSinglePostSectionName($sectionParentCat) {
    $args = array(
        'hide_empty' => 0,
        'parent' => $sectionParentCat,
    );

    $singleSidebarCategories = get_categories($args);
    $thisPostCategories = wp_get_post_categories(get_the_ID());
    foreach ($singleSidebarCategories as $category) {
        foreach ($thisPostCategories as $thisPostCategory) {
            if ($category->cat_ID == $thisPostCategory) {
                return get_category($thisPostCategory)->name;
            }
        }
    }
}

function getSinglePostSectionURL($sectionParentCat) {
    $args = array(
        'hide_empty' => 0,
        'parent' => $sectionParentCat,
    );
    ...
    foreach ($singleSidebarCategories as $category) {
        foreach ($thisPostCategories as $thisPostCategory) {
            if ($category->cat_ID == $thisPostCategory) {
               return get_category_link( $thisPostCategory );
            }
        }
    }
}

function getSinglePostSectionID($sectionParentCat) {
    $args = array(
        'hide_empty' => 0,
        'parent' => $sectionParentCat,
    );
    ...
    foreach ($singleSidebarCategories as $category) {
        foreach ($thisPostCategories as $thisPostCategory) {
            if ($category->cat_ID == $thisPostCategory) {
               return $thisPostCategory;
            }
        }
    }
}

// Sections Menu Shortcode
function menu_shortcode($atts){  
    $menu_name = $atts['name'];   
        echo '<div class="sections-list-wrapper">';
	wp_nav_menu($args = array('menu' => $menu_name));  
        echo '</div><div class="clearfix"></div>';
}
add_shortcode( 'sections-menu', 'menu_shortcode' );
